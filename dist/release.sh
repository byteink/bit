...
# Stamp the version into a STAGED COPY of selfhost/, never into the source tree.
#
# `selfhost/version.bit` holds the checked-in development version, and the
# self-hosted compiler compiles that constant directly — so building selfhost/ as
...
STAGE_SRC="${OUT}/stagesrc/selfhost"
mkdir -p "${STAGE_SRC}"
find selfhost -maxdepth 1 -name '*.bit' ! -name 'version.bit' -exec cp {} "${STAGE_SRC}/" \;
printf '// Generated by dist/release.sh. Source of truth: selfhost/version.bit.\nconst bitVersion: string = "%s"\n' \
...
real="$(find selfhost -maxdepth 1 -name '*.bit' | wc -l | tr -d ' ')"
[ "${staged}" = "${real}" ] || {
