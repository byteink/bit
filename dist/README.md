...
component is the pinned Zig toolchain (`.zigversion`) that runs `build.zig`,
listed under `metadata.tools`. Since #1593 the compiler itself is bootstrapped
by the pinned stage0 (`dist/stage0/SHA256SUMS`), which is a Bit release, not a
third-party component. The document states the absence of vendored dependencies
