...
`bit-<version>.cdx.json` is a CycloneDX 1.6 SBOM, one per release (not one per
target — the three archives share the same toolchain and dependency set).
Generated by `dist/sbom.py` (task #1752) via `cyclonedx-python-lib`, never
hand-written. `bit` has no third-party source and `build.zig.zon` declares no
`.dependencies`, so the only real external component is the pinned Zig seed
toolchain (`.zigversion`), listed under `metadata.tools`. The document states
the absence of vendored dependencies explicitly rather than shipping an empty
component list.
