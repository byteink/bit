...
`unreachable-code` (E0212) is grouped with the phase-2 rules below by number
— it was assigned its code before the dependency analysis that showed it
...
```bit
// selfhost/lower.bit — Self-host Stage 2 (#1337): AST -> SSA IR lowering.
// bit:lint max-file-lines=6869 -- known debt tracked by #1689 (5-8x the
