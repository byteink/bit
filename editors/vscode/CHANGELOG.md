...
- `bit.json` is now associated with the `jsonc` language (it allows `//` and
  ...
  layer), so opening one no longer shows every comment as a JSON
...
- Added JSON schemas for both files under `schemas/`, wired through
  ...
  `bit.lock` records per resolved dependency. `bit.lock`'s schema
...
- Removed `bit.formatOnSave`. `bit lsp` now advertises
  ...
  so Format Document works in VS Code without the extension having
