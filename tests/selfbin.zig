//! STATUS (#1591): this file survives as a HELPER LIBRARY, not as a harness.
//! Its `privateCopy`/`release` are called by tests/{imports,diffimports,version}.zig,
//! which are seed-coupled and die with seed/ (#1593). Delete it with them, not before.
//!
//! The PROPERTY it documents is separately gated, in Bit, by tests/bit/selfbin.bit
//! (48 checks) — because the hazard is NOT Zig's. compiler/build.bit:400 publishes
//! the linked binary with a truncating writeFile + fsChmod and no temp-then-rename,
//! so the Bit toolchain reproduces #1644 exactly. It did so twice on 2026-07-29:
//! tests/bit/stress.bit copied a partial file and failed with no output, and
//! tests/bit/lintcmd.bit exec'd one and died with SIGSEGV and empty stderr. Both
//! were worked around with build-step dependencies; the non-atomic publish remains.
//!
