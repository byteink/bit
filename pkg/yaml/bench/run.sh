...
render() {
  local sha bitver gover host fsize yamlv3ver goccyver
  ...
  fsize=$(stat -f%z "$DATA")
  yamlv3ver=$(pinnedVersion yamlv3)
  goccyver=$(pinnedVersion goccy)
  ...
  local md=$OUT/block.md
  {
    ...
    echo
    echo "> pkg/yaml at \`${sha}\`, compiled by the released ${bitver}. Go ${gover}, gopkg.in/yaml.v3 ${yamlv3ver}, goccy/go-yaml ${goccyver}, host: ${host}."
