...
Status: normative. This is the canonical form `bit fmt` produces and the
form the repository's formatting gates hold it to.
...
this down: each rule must be turned into a gate, not left as prose. Each rule
states, in one phrase, how a checker would test it. Where the code has a
settled rule but the corpus (a corpus-wide inventory of the formatter's
actual output) also found a *bug* the rule does not yet hold to in every
case, that bug is out of scope here - a bug is fixed, not codified - and is
named as open work instead.
...
helpers), `compiler/fmtwrap.bit` (the width-aware wrap-decision machinery -
flat lists, bracketed comma lists, and binary/logical chain flattening),
`compiler/fmtdispatch.bit` (declarations and
...
**Settled for every construct that renders as a list or a chain.** A bare
binary/logical/string-concat chain and a flat unbracketed list use the same
flat-then-fits-or-wrap shape `fmtPrintCommaList` uses for a bracketed list -
all three share one implementation pattern, `compiler/fmtwrap.bit:1-7`,
including the interaction where a binary chain is nested as a bracketed
list's sole item (see §5). §10 records the exemptions that are the whole of
what is left unsettled; there is currently no construct with zero width
check.
...
`asm` blocks are a case worth stating explicitly: their `x64`/`arm64`/
`result`/`clobber` sub-clauses are sorted by their own `span.start` before
printing, with the gap flushed explicitly before each clause (`fmtAsmStmt`,
`compiler/fmtasm.bit:86-136`), so a clause never prints ahead of its true
source position and sweeps up a neighboring clause's still-unconsumed
comment. This is what makes "attached to the same node it was attached to in
the source" hold universally rather than for every node kind except `asm`.
...
followed in the source - the same per-file comparison technique used for
`asm` regions, generalized to any node kind.

## 5. Bracketed comma lists (params, args, class/field/variant lists, generics, composite literals, imports, tuples)

**Rule** (`fmtPrintCommaList`, `compiler/fmtwrap.bit:158-299`, settled): the
list flattens onto one line only if **both** hold -
...
**A trailing same-line comment is OUTSIDE the width budget.** No
...
the measurement the exemption above exists to suppress. The two
...
Any gate that measures line width must exempt the trailing comment to agree
with this rule.
...
(idempotence checked over the whole corpus).

## 6. Inline vs. stacked bodies - settled
...
**No single rule works here, because the tree legitimately uses both
spellings on purpose.** A one-line accessor is clearest inline, while a
function whose single statement is itself a multi-line `match` is mangled by
being forced onto one line. The formatter bends to preserve the author's own
choice for functions, while match arms always collapse, since there the
single expression *is* the point of writing it as a `match` at all. **Do not
collapse function bodies to a single always-collapse rule by instinct
because match arms look inconsistent next to them - that inconsistency is
the settled rule, not an oversight.**
...
2. **Style parens, independent of precedence** - a binary operand that mixes
   ...
   parenthesized too, so `a || b && c` formats to
...
(`fmtAsmStmt`, `compiler/fmtasm.bit:86-136`, see §4). The block is
width-aware like any other bracketed construct (§1): it renders on one line
only if that fits under the 100-column budget and holds no comment, via a
trial render (`fmtAsmFlatSlots`, `compiler/fmtasm.bit:75-84`); otherwise each
clause - `x64 { ... }`, `arm64 { ... }`, `input ...`, etc. - gets its own
line. A clause whose own content (e.g. a long byte-array code list) wraps
internally never shares a line with a neighboring clause's opening or
closing brace (never the assembled bytes - `AsmCode`'s items are a plain
comma-separated integer list, parsed independent of line breaks).
...
and after. For a block whose flat form doesn't fit, each clause lands on its
own output line, with no clause's content sharing a line with a neighboring
clause's opening or closing brace.
...
## 10. Width budget for bare (non-bracketed) constructs - settled
...
the same rule §1 and §5 state, extended to every construct that had none.
The one interaction the extension needs: a chain nested as a single-element
bracketed list's sole item (§5's "Nested width decisions") measures against
the indent baseline rather than the real column, because the enclosing
single-element list was always going to stay flat regardless, and measuring
for real would only manufacture a wrap that forces every enclosing
single-element list to explode for no shortening gain.

### Break-point preference for a binary chain
...
This section is kept on its own, rather than folded into §1, because it is
where a reader checking whether a *specific* non-bracketed construct is
covered should look first - nothing here is exempt from §1 any longer.

No other category considered here was found unsettled: indentation (§2),
the blank-line policy between declarations (§3), and whether the formatter
may move a comment (§4) each have a specific rule in the printer today,
cited above.


## 11. `bit fmt` owns every file - there is no exemption class
...
actually contains. Contrast a *ceiling* (`test-fmt-ceiling`, which ratcheted the trees
`bit fmt` had not yet reached): a ceiling says "not yet," and the gate can
only ever prove the count shrinks. An ignore list says "never," and nothing
ever checks that claim again. The two read as similar and are not: **ignore
means never, ceiling means not yet.** `test-fmt-ceiling` proved the point by
reaching zero and being retired - the whole tree was reformatted once and
replaced with a zero-tolerance gate (`test-fmt-strict`,
`_tests_/bit/fmtzerocheck.bit`) naming its residual debt by file rather than
by count.
...
**The fix is not an exception, and its verification set is exactly these
six files:** `runtime/stw/stwpoll.bit`,
`runtime/root/linux/boottail.bit`, `runtime/sched/switch.bit`,
`runtime/sched/task.bit`, `runtime/sched/timer.bit`,
`runtime/sched/windows/spawn.bit`. `bit fmt` produces the alignment these
files were hand-maintaining, and they are ordinary formatted files with no
special handling anywhere - the same six files that made this section
necessary are the ones that make it unnecessary now that the formatter can
do the one thing it was missing.
...
none of it a table) - real, but nothing for the fix above to address and
nothing to exclude either. **The two groups were separated by measurement,
not by assumption**: an earlier draft of this decision conflated them into one
...
tracked as ordinary, ungated debt by the `test-fmt-ceiling` ratchet.

The whole tree was reformatted once: `grow.bit`, `queue.bit`,
...
above for the six comment-alignment files once the alignment fix landed.
`sleep.bit` and `runtime/root/windows/boot.bit` were already canonical by
that point (fixed independently of this list; this section's "fails
...
`test-fmt-strict`'s debt list.

**Check:** `bit fmt --check` on the six files named above exits 0; formatting
each of the six twice produces byte-identical output - `bit fmt` stays
idempotent even while computing a column from the widest line in a run.

## 12. Trailing-comment column alignment
...
assumed.** gofmt is this feature's stated model, so its own
...
This aligns `muState`/`muFrameLong` to each other and `muAcked`/`muOwnerVeryLong`
...
## 13. JSX elements and fragments (SPEC §12.12)
