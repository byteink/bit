...
> Peak RSS above is a within-run median like every other figure in that table, but it can still swing further ACROSS separate regenerations than one run shows. From every regeneration recorded in `bench/history.csv`, restricted to the same four loose rows above and to the Go/C columns (the Bit column reflects real compiler/runtime changes over that history, not noise): `alloc` c: 1.5-2.0 MB (median 1.6 MB, N=27); `allocflat` c: 1.5-1.9 MB (median 1.5 MB, N=18); `strings` go: 64.3-90.9 MB (median 69.2 MB, N=24). Read that cell's published number as representative of the stated range, not a fixed constant.
