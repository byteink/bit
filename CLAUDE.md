...
- **A hang is a failure, not a stall (#1637, #1652).** Every subprocess a harness spawns carries a wall-clock deadline from `tests/proc.zig` - the golden (`tests/harness.zig`), stress (`tests/stress.zig`), examples (`tests/examples.zig`), imports (`tests/imports.zig`) and orphaned-test (`tests/testroots.zig`) harnesses, covering both compilers' builds and every executed binary. (`tests/gcdiff.zig` spawns nothing; it runs the Zig collector in-process.) Exceeding it kills that child (its own PID, never a name pattern), reports `TIMED OUT` naming the case, and reddens the suite. Default **900s** — a stopgap, see below; override with `BIT_TEST_TIMEOUT_S=<seconds>` on a slower host, or `0` to disable and block forever as before. It was 300s, ~6x the corpus's measured worst case (`tests/stress/quicwire` under `BIT_GC=stress`, 48.6s at load 14), until #1849: the stress-GC path regressed 4-9x after `libbitrt.a` became all-Bit (`quicwire` 38.3s→161s, `polloneshotdarwin` 13.1s→123s, both still byte-correct), which cut the margin to 1.9x and made `zig build test` nondeterministically red — a different program each run. **Do not read a `TIMED OUT` here as a hang until you have timed the program standalone**: the harness prints "This is a HANG, not an output mismatch" for programs that finish correctly in 123s. #1849 restores the runtimes and puts this back to 300s. A timeout is a distinct outcome from a crash: a child killed by SIGSEGV/SIGBUS/SIGABRT is still reported as a crash naming the signal.
