...
# them: bitc2 once typed `mapped<i64,i64>(...)`'s result as `[]T` with `T`
...
# emit a binary) and is reported but not fatal — the fatal ones are DIFF (bitc2
...
for d in examples/*/; do
  ...
  if ! "$BITC2" build "$d" -o "$TMP/b2_$n" >"$TMP/err_$n" 2>&1; then
  ...
  if [ "$se" != "$b2" ] || ! cmp -s "$TMP/o_seed_$n" "$TMP/o_b2_$n"; then
    echo "DIFF      $n (exit seed=$se bitc2=$b2)"
