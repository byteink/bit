...
# the seed rejects at check time are skipped (bitc2's checker is still partial).
...
for f in $(find stdlib examples tests/cases tests/imports -name '*.bit' | sort); do
  ...
  b2=$(perl -e 'alarm 10; exec @ARGV' "$BITC2" --dump-types "$f" 2>/dev/null)
  if [ "$seed" = "$b2" ]; then
    match=$((match + 1))
  else
    mismatch=$((mismatch + 1))
...
if [ -n "$firstbad" ]; then
  ...
  diff <("$SEED" --dump-types "$firstbad" 2>/dev/null) <(perl -e 'alarm 10; exec @ARGV' "$BITC2" --dump-types "$firstbad" 2>/dev/null) | head -12
fi
