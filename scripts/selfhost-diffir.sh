...
for f in $(find stdlib examples tests/cases tests/imports -name '*.bit' | sort); do
  ...
  b2=$(perl -e 'alarm 10; exec @ARGV' "$BIT2" --dump-ir-pre "$f" 2>/dev/null)
  if [ "$seed" = "$b2" ]; then
    match=$((match + 1))
  else
    mismatch=$((mismatch + 1))
...
if [ -n "$firstbad" ]; then
  ...
  diff <("$SEED" --dump-ir-pre "$firstbad" 2>/dev/null) <(perl -e 'alarm 10; exec @ARGV' "$BIT2" --dump-ir-pre "$firstbad" 2>/dev/null) | head -20
fi
