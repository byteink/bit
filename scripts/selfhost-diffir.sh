...
for f in $(find stdlib examples tests/cases tests/imports -name '*.bit' | sort); do
  ...
  want=$(perl -e 'alarm shift; exec @ARGV' "$TIMEOUT" "$ORACLE" --dump-ir-pre "$f" 2>/dev/null)
  rc=$?
  if [ "$rc" -eq 142 ]; then
...
echo "IR (pre-opt) differential: MATCH=$match MISMATCH=$mismatch NO-VERDICT=$timeouts ORACLE-TIMEOUT=$oracletimeouts ORACLE-CRASH=$oraclecrashes SKIP(lower/check-err)=$skip"
...
if [ -s "$work/mismatch" ]; then
  ...
  head -3 "$work/mismatch" | while read -r f; do
    ...
    diff <(canon_ir_ids "$(perl -e 'alarm shift; exec @ARGV' "$TIMEOUT" "$ORACLE" --dump-ir-pre "$f" 2>/dev/null)") \
