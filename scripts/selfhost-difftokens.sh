...
# both the Zig seed (`bitc`) and the Bit compiler (`bitc2`) and diff their
...
for f in $(find stdlib examples tests/cases tests/imports -name '*.bit' | sort); do
  ...
  b2=$("$BITC2" --dump-tokens "$f" 2>/dev/null)
  if [ "$seed" = "$b2" ]; then
    match=$((match + 1))
  else
    mismatch=$((mismatch + 1))
...
if [ -n "$firstbad" ]; then
  ...
  diff <("$SEED" --dump-tokens "$firstbad" 2>/dev/null) <("$BITC2" --dump-tokens "$firstbad" 2>/dev/null) | head -20
  exit 1
fi
