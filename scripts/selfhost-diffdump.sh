#!/usr/bin/env bash
# Table-driven differential driver (#2743) behind the six shims
# selfhost-diff{ast,tokens,diags,types,ir,iropt}.sh, which `exec` into this
# file with a form name (the six `case` arms below, one per row). BOTH sides
# of every comparison are alarm-guarded (perl `alarm`) -- see
# selfhost-diffcheck.sh's header for why the oracle side needs a bound too.
#
# `types`/`ir`/`iropt` never run the resolver, so a bug that only exists on
# the resolver-active path (bit build/run/test/check/doc) is invisible to
# these three rows however MATCH they read -- see docs/development.md "What
# the differentials assert" and #3069 for the full account.
#
# Full design record (per-form history #1332-#1339, reading a red
# types/ir/iropt run as oracle-lag vs. regression #2382, and #3125's
# signature-explained divergences) moved to
# docs/development/dump-differential.md (#4265, to stay under the 800-line
# ceiling). Read it before changing any invariant below.
#
