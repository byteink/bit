...
[ -x "$BITC" ] || { echo "$BITC not built (run: zig build)" >&2; exit 1; }
...
for c in $CASES; do
  ...
  i=0; while [ "$i" -lt "$CRUNS" ]; do
    t0=$(now); "$BITC" build "$d/$c.bit" -o "$WORK/$c.bit" >/dev/null; t1=$(now)
...
"$BITC" build bench/cases/startup/startup.bit -o "$WORK/s.bit" >/dev/null
...
md="$WORK/results.md"
{
  ...
  echo "> Method: median of ${RUNS} runs. C built \`cc ${CFLAGS}\`, Go \`go build\`, Bit \`bitc build\` — each language's standard optimized build."
